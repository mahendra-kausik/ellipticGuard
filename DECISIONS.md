...
## D-028 — Layer 11: the deployed container loads exported weights via `MODEL_URI`, not the MLflow registry
- **Date / Layer:** Layer 11
- **Context:** D-024 left Layer 11 to resolve why a Linux container can't load `models:/elliptic-illicit@production` from the local sqlite store. Investigating it properly surfaced that the problem has **two** independent halves, and the one D-024 named is the smaller: (1) `mlflow.db` bakes absolute Windows paths (`file:C:/Users/2kaus/.../mlruns/...`) into `model_versions.source` and `logged_models.artifact_location`, so MLflow reads a literal `C:/Users/...` string on a Linux filesystem; and (2) **`mlruns/` and `mlflow.db` are both git-ignored, and Hugging Face Spaces builds from a git repo** — so the container never receives the registry at all, absolute paths or not. The owner asked directly why we don't just commit `mlruns/`; the honest answer is that committing it fixes half (2) and leaves half (1) fully intact, so it isn't a fix on its own — it's a prerequisite for a DB-rewrite we'd still have to do.
- **Decision:** `pipelines/export_model.py` resolves the `@production` alias **on the machine that owns the registry** (via the same `get_model_version_by_alias` call the app already uses, so the exported weights and the recorded version cannot disagree) and downloads the artifact to `api/model/` (~1.1 MB), which **is** committed. `src/serving/app.py` gains two env overrides: `MODEL_URI` (default unchanged: `models:/{REGISTRY_NAME}@production`) and `MODEL_VERSION` (default unchanged: the alias lookup). `api/Dockerfile` copies `api/model/` and sets `MODEL_URI=/app/model MODEL_VERSION=2`; it no longer copies `mlflow.db`/`mlruns/`. `mlruns/` stays git-ignored.
- **Why:** It ships the ~1 MB that serving actually needs instead of 13 MB of regenerable run history, and it sidesteps the path problem rather than patching it — a directory handed to the app via `MODEL_URI` has no stale absolute path recorded anywhere. Local behaviour is unchanged (both env vars are unset locally, so the registry stays the source of truth and Layer 7's gate still holds as written). The alias still governs *what* ships, so promotion remains the single control point.
- **Alternatives Considered:** **Commit `mlruns/` + `mlflow.db` and rewrite the stored paths to be relative** — rejected: hand-editing a registry DB is exactly what D-024 flagged as risky, it puts a growing binary SQLite file in git (merge conflicts on every run), and it silently re-breaks on any Windows re-train unless the rewrite is scripted into the pipeline. **Re-log/re-register v2 against a tracking URI that records container-relative paths** — genuinely cleaner, and it would keep a real registry lookup inside the container, but it needs a re-registration pass plus verification that v2's identity survives, which is more risk and work than the deployment gate justifies. **Hosted MLflow tracking server** — the actually-correct production answer (and still the documented upgrade), but no free tier supports it, so it violates the free-tier constraint.
- **Tradeoffs / risks:** **The deployed container loads a path, not the registry — a real step back from Layer 7's "loads Production from the registry, not a hardcoded path" story, stated plainly in README.md's Limitations rather than glossed.** Layer 7's gate remains satisfied locally, but an interviewer asking "is the live Space using your registry?" must get an honest no. Concretely: (a) the exported weights are a build artifact that can go stale if someone promotes a new version and forgets to re-run the export — mitigated by `export_model.py` printing the resolved version and deleting any prior export, but not enforced; (b) `MODEL_VERSION=2` is pinned literally in the Dockerfile and must be updated on promotion — the export script prints a reminder; (c) a new test (`test_serves_from_exported_weights_without_registry`) makes every `MlflowClient` construction fail loudly, so the container's no-registry code path is proven rather than assumed; (d) the exported `MLmodel` ships with `artifact_path: file:C:/Users/.../mlruns/...` — vestigial (loading resolves `model.ubj` relative to the directory), but it would publish a local filesystem path to a public Space and re-raise the very confusion this export escapes, so `export_model.py` strips that line and the export is verified free of local paths. The clean removal of all of this is a hosted tracking server, at which point both env vars simply go unset.
- **Also decided (repo layout):** `api/Dockerfile` moved to the repo **root**. HF Docker Spaces require the Dockerfile at the root and the path is not configurable (verified against HF's Docker Spaces docs, not assumed), and the Space is configured by a YAML block at the top of the root `README.md` (`sdk: docker`, `app_port: 7860` — our port choice was already right). `PROJECT_PLAN.md` §5's `api/ # Dockerfile + app entry` layout predates that constraint; §5 was amended to match reality rather than worked around, and `api/` now holds the exported weights. Alternatives rejected: a second deploy-only repo whose root has the Dockerfile (honors §5 exactly, but two repos to keep in sync for a one-file placement issue); a duplicate root Dockerfile (guaranteed drift). Frontmatter in `README.md` renders as a small metadata table on GitHub — accepted so one repo serves both GitHub and the Space with no divergence.
- **Supersedes:** D-024 (specifically its item 3, the deferred Docker artifact-path limitation; D-024's items 1 and 2 — raw probability, alias-based promotion — still stand unchanged)
